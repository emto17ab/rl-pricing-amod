"""
Calculate hourly average wage per individual for Washington DC downtown zip codes
...
# Washington DC downtown zip codes
DC_ZIP_CODES = [
    '20001',  # Mt. Vernon Square / U Street
    '20002',  # Near Northeast / Capitol Hill
    '20003',  # Capitol Hill / Navy Yard
    '20004',  # Downtown / Penn Quarter
    '20005',  # Downtown / Dupont Circle
    '20006',  # Downtown / Foggy Bottom
    '20007',  # Georgetown
    '20008',  # Cleveland Park / Woodley Park
    '20009',  # Adams Morgan / Columbia Heights
    '20010',  # Columbia Heights / Park View
]

# Income brackets and their midpoints
INCOME_BRACKETS = [
...
# Load census data (2019)
print("Loading 2019 census data...")
s1901_df = pd.read_csv("Trip Data/ACSST5Y2019.S1901-Data.csv", skiprows=[1])
s1902_df = pd.read_csv("Trip Data/ACSST5Y2019.S1902-Data.csv", skiprows=[1])

# Filter for DC zip codes
dc_geo_ids = [f"8600000US{zip_code}" for zip_code in DC_ZIP_CODES]
s1901_dc = s1901_df[s1901_df['GEO_ID'].isin(dc_geo_ids)].copy()
s1902_dc = s1902_df[s1902_df['GEO_ID'].isin(dc_geo_ids)].copy()

# Merge the dataframes (2019 uses S1902_C01_019E for population)
merged_df = pd.merge(s1901_dc, s1902_dc[['GEO_ID', 'S1902_C01_019E', 'S1902_C01_001E']], 
                     on='GEO_ID', how='inner')

# Add zone index
merged_df['zone_index'] = merged_df['GEO_ID'].apply(
    lambda x: DC_ZIP_CODES.index(x.replace('8600000US', ''))
)
merged_df = merged_df.sort_values('zone_index')

# Calculate household size
merged_df['households'] = pd.to_numeric(merged_df['S1901_C01_001E'], errors='coerce')
merged_df['population'] = pd.to_numeric(merged_df['S1902_C01_019E'], errors='coerce')  # 2019 format
merged_df['household_size'] = merged_df['population'] / merged_df['households']

print(f"\nProcessing {len(merged_df)} Washington DC zip codes...")
...
with open('dc_wage_data.json', 'w') as f:
    json.dump(output_data, f, indent=2)

print(f"\n✓ dc_wage_data.json created successfully")

# Calculate overall weighted average hourly wage
print("\n" + "="*80)
print("WEIGHTED AVERAGE HOURLY WAGE ANALYSIS - WASHINGTON DC")
print("="*80)
...
for idx, row in merged_df.iterrows():
    zone_idx = row['zone_index']
    zip_code = DC_ZIP_CODES[zone_idx]
    households = row['households']
